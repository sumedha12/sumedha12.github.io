...
function avail(friend) 
{
    var friends =friend;
    db.get(friends).then(function (doc){    
    if(spec_friends.length!==0){
        for(var i =0 ;i<spec_friends.length;i++)
            {
                if(spec_friends[i]==friends)
                {
                    alert("found");
                }
            }
...
db.get(x).then(function (doc) {
    ...
    var i;
    for(i=0;i<len;i++)
    $(".well").append("<a href ='#' onclick='avail("+doc.Friends[i]+")' class='btn btn-default' > "+doc.Friends[i]+" </a> <br><br>");          
    return doc;
}).catch(function(err) {
  console.log(err);
});  
...
db.get(x).then(function (doc) {
   ...
   for(i=0;i<len;i++)  
    {                
    $('.well').append('<div> <label id ="checkbox" class="checkbox-inline"><input type="checkbox" id= "user" name="user" value='+doc.Friends[i]+'>'+doc.Friends[i]+'</label></div>');                
    }
    $('.well').append('</fieldset>');
    var glen = doc.glen;    
    for(i=0;i<glen;i++)
    {
        $("ul").append("<li class='list'>"+doc.group[0][i]+"</li>");     
    }
    addOptionListeners();    
    return doc;
}).catch(function (err) {
...
var x = x.replace("?", "");     
db.get(x).then(function (doc) {
var groupname = document.getElementById("menu1").name;
var groupuser = [];
var no;
